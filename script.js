document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            const response = await fetch('https://sole-masha-dniel23-aa38c4ec.koyeb.app/gerar-pagamento', {
                method: 'POST',
                headers: {
    ...
    function verificarStatusPagamento() {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                const response = await fetch(`https://sole-masha-dniel23-aa38c4ec.koyeb.app/status-pagamento/${currentPaymentId}`);
                const data = await response.json();
                ...
                if (data.status === 'approved') {
                    ...
                    btnDownload.onclick = () => {
                        ...
                        window.open(`https://sole-masha-dniel23-aa38c4ec.koyeb.app/gerar-certificado?id=${currentPaymentId}`, '_blank');
                    };
                }
            } catch (error) {
